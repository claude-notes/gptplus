...
assert.match(codexHome, /<a href="https:\/\/openai\.com\/codex\/get-started\/"[^>]*>Codex 入门步骤<\/a>/);
...
assert.match(codexHome, /<section class="platform-guide"[\s\S]*Windows、macOS 和手机怎么用 Codex/);
assert.match(codexHome, /<table class="platform-matrix"/);
assert.match(codexHome, /<th scope="col">设备<\/th>[\s\S]*<th scope="col">使用方式<\/th>[\s\S]*<th scope="col">小白需要准备<\/th>/);
assert.match(codexHome, /<th scope="row">Windows<\/th>/);
assert.match(codexHome, /<th scope="row">macOS<\/th>/);
assert.match(codexHome, /<th scope="row">iPhone 与安卓手机<\/th>/);
assert.match(codexHome, /本站提供独立的第三方远程安装与使用指导/);
assert.match(codexHome, /与 OpenAI 无隶属或官方合作关系/);
assert.match(codexHome, /"datePublished": "2026-07-14"/);
assert.match(codexHome, /"@type": "SoftwareApplication"/);
assert.match(codexHome, /"sameAs": "https:\/\/openai\.com\/codex\/get-started\/"/);
assert.match(codexHome, /"citation": \[/);
assert.match(codexHome, /https:\/\/help\.openai\.com\/en\/articles\/11369540-using-codex-with-your-chatgpt-plan/);
...
assert.equal(
  (codexHome.match(/<img src="wechat-qr\.jpg"[^>]*loading="lazy"[^>]*decoding="async"/g) ?? []).length,
  2,
  'Both Codex QR-code images should be lazy-loaded and asynchronously decoded',
);
...
assert.match(codexCss, /\.platform-guide/);
assert.match(codexCss, /\.platform-matrix/);
assert.match(codexCss, /\.service-disclosure/);
assert.match(codexCss, /@media \(max-width: 720px\)[\s\S]*\.platform-matrix/);
